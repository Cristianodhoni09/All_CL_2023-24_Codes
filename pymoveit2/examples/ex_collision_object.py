...
def main():
    ...
    node.declare_parameter("position", [0.55, 0.07, -0.60])
    ...
    if "add" == action:
        node.get_logger().info(f"Adding collision mesh '{filepath}' {{position: {list(position)}, quat_xyzw: {list(quat_xyzw)}}}")
        moveit2.add_collision_mesh(filepath=filepath, id=mesh_id, position=position, quat_xyzw=quat_xyzw, frame_id=ur5.base_link_name())
    else:
